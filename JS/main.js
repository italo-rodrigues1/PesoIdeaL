...
form.addEventListener('submit', updatSubmit);

//FUNÇÃO PRINCIPAL
function updatSubmit(event) {
  ...
  const layout = `
    <h2>Resultado do Senhor(a): ${name}</h2>
...
//FUNÇÃO PARA TRANSFORMAR O SELECT EM NÚMERO
function getSelectedValue(id) {
  const select = document.getElementById(id);
  return select.options[select.selectedIndex].value;
}
//FUNÇÃO PARA TRANSFORMAR O TIPO STRING EM NÚMERO
